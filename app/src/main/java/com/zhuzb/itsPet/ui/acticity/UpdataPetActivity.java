...
public class UpdataPetActivity extends Activity {
    @ViewInject(value = R.id.pet_tab)
    TextView tabView;
    ...
    private UpdataPetItem updataPetItem;
    private int petId;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Intent i = getIntent();
        updataPetItem = (UpdataPetItem) i.getSerializableExtra( "data" );//用户信息
        x.view().inject( this );
        tabView.setText( "修改宠物资料" );
        baocunBT.setEnabled( true );
        petId = (int) updataPetItem.getPet_id();
        initData( updataPetItem );
    }

    private void initData(UpdataPetItem data) {
        Picasso.with( getBaseContext() ).load( data.getPet_user_img_url() ).into( petImgView );//头像
        etName.setText( data.getPet_name() );
        if ("母".equals( data.getPet_sex() )) {
            spinnerView.setSelection( 1 );    //根据该选项的位置设置该选项为spinner默认值
        } else {
            spinnerView.setSelection( 0 );    //根据该选项的位置设置该选项为spinner默认值
        }
        etPinzhong.setText( data.getPet_breed() );
        tvTime.setText( data.getPet_year() + "年" + data.getPet_month() + "月" + data.getPet_day() + "日" );
        etBeiZu.setText( data.getPet_bz() );
        etFengSi.setText( data.getFollow_name() );
    }
    ...
    private void addPetBT() {
        ...
        if (petPZ.length() == 0) {
            ...
            map.put( "petBZ", petBZ );
            map.put( "petId", String.valueOf( petId ) );
            if (!isSend) {
                sendPet(  map );
            }else {
                sendPet( fileImg, map );
    ...
    private void sendPet(final File file, final Map<String, String> params) {
        ...
        new Thread( new Runnable() {
            @Override
            public void run() {
                OkhttpUtils.sendOkHttpRequestFromdata( CONSTS.UPDATA_PET_URL,
                        file,
                        params,
     ...
     * 修改宠物
     */
    private void sendPet(final Map<String, String> params) {
        showDialg( "正在修改宠物资料..." );
        baocunBT.setEnabled( false );
        new Thread( new Runnable() {
            @Override
            public void run() {
                OkhttpUtils.sendOkHttpRequestFromdata0( CONSTS.UPDATA_PET_URL,
                        params,
                        new okhttp3.Callback() {
                            @Override
                            public void onFailure(Call call, final IOException e) {
    ...
    private void errorToast(final String error) {
        UpdataPetActivity.this.runOnUiThread( new Runnable() {
            @Override
            public void run() {
                Toast.makeText( UpdataPetActivity.this, error, Toast.LENGTH_SHORT ).show();
            }
        } );
    }
}
